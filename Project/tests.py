...
def compareAnalytical1D():
	...
	"""
	with open('compareAnalytical1D.txt', 'a') as out:
		#use randomly selected fixed values
		D_0 = randRange(0, 20)
		Sigma_a_0 = randRange(0, 20)
		S_0 = randRange(0, 20)
		
		M = 128 # y cell count == number of rows
		N = 128 # x cell count == number of columns
		
		width = [0.1 for i in range(N)] #width of each column at each mesh x index
...
def main():
	print("running comparison to analytical solution...")
	err = compareAnalytical1D()
